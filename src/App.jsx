...
import { Plus, Minus, X, Upload, ArrowUp, ArrowDown, Trash2, Loader2, FolderOpen } from 'lucide-react';
...
const QUALITIES = [
  { key: 'base', label: 'Базовые навыки', short: 'База', accent: '#2dd4c9', accentDim: '#0d5c54', glow: 'rgba(45,212,201,0.35)' },
...
export default function App() {
  ...
  const persistLibrary = useCallback(async (next) => {
    ...
    if (!res) showToast('Не удалось сохранить библиотеку');
  }, []);

  const persistLayout = useCallback(async (next) => {
    setLayout(next);
    const res = await storage.set(LAYOUT_KEY, JSON.stringify(next));
    if (!res) showToast('Не удалось сохранить настройки секций');
  }, []);
  ...
  const persistBoard = useCallback((next) => {
    ...
    boardSaveTimer.current = setTimeout(async () => {
      ...
      if (!res) showToast('Не удалось сохранить прогресс');
    }, 500);
  }, []);
  ...
  const handleFileChosen = async (file) => {
    ...
    try {
      ...
      const name = file.name.replace(/\.[^.]+$/, '').slice(0, 40) || 'Навык';
  ...
  const changeCellCount = (qualityKey, delta) => {
    ...
    if (delta < 0) {
      ...
      if (board[cellId]) {
        const ok = window.confirm('В этой ячейке есть навык. Удалить вместе с ней?');
        if (!ok) return;
        const nb = { ...board };
        delete nb[cellId];
        persistBoard(nb);
  ...
  const deleteSkill = (id) => {
    const ok = window.confirm('Удалить эту иконку из библиотеки? Она пропадёт из всех ячеек, где выбрана.');
    if (!ok) return;
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0e1a', color: '#7c88a8', fontFamily: 'Inter, sans-serif' }}>
        <Loader2 className="spin" size={22} style={{ marginRight: 8 }} /> Загрузка…
      </div>
    );
  }
  ...
  return (
    <div className="sc-root">
      <style>{`
        ...
        .sc-topbar { display: flex; align-items: center; justify-content: space-between; margin-bottom: 22px; gap: 10px; }
        ...
        .sc-values { background: #0c1220; border: 1px solid #1c2540; border-radius: 8px; padding: 4px 5px; display: flex; flex-direction: column; gap: 3px; }
        .sc-vrow { display: flex; align-items: center; gap: 3px; }
        .sc-vlabel { font-size: 8px; color: #5b6784; width: 14px; flex-shrink: 0; }
      ...
      <div className="sc-topbar">
        <div>
          <div className="sc-heading sc-title">Конструктор навыков</div>
          <div className="sc-subtitle">Нажми на ячейку, чтобы выбрать навык</div>
        </div>
        <div className="sc-libbtn" onClick={() => setLibraryOpen(true)}>
          <FolderOpen size={15} /> Библиотека
        </div>
      </div>
      ...
      {QUALITIES.map((q) => {
        ...
        return (
          <div key={q.key} className="sc-section" style={{ '--acc': q.accent, '--acc-dim': q.accentDim, '--acc-glow': q.glow, '--acc-bg': q.accentDim + '22' }}>
            ...
            {count === 0 ? (
              <div className="sc-empty-msg">Ячеек нет — добавь через «+» выше</div>
            ) : (
                      ...
                      {skill && showValues && (
                        <div className="sc-values">
                          <div className="sc-vrow">
                            <span className="sc-vlabel">об</span>
                          ...
                          {hasPotential && (
                            <div className="sc-vrow">
                              <span className="sc-vlabel">пот</span>
                          ...
                          <div className="sc-addpot" onClick={() => togglePotential(cellId, !hasPotential)}>
                            {hasPotential ? '− потенциал' : '+ потенциал'}
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <div className="sc-footer">
        Small details → Big difference · <span className="sc-reset" onClick={async () => {
          const ok = window.confirm('Удалить все ячейки, библиотеку и настройки? Это необратимо.');
          if (!ok) return;
        ...
        <div className="sc-overlay" onClick={() => setPicker(null)}>
          <div className="sc-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {library.filter((s) => s.quality === picker.quality).length === 0 ? (
              <div className="sc-empty-msg">Для этого качества пока нет иконок в библиотеке.<br />Загрузи первую ниже.</div>
            ) : (
            ...
            <div className="sc-uploadbtn" onClick={() => openUploadFor('picker', picker.quality, picker.cellId)}>
              <Upload size={15} /> Загрузить новую иконку
            </div>
          </div>
        </div>
      )}
      ...
      {libraryOpen && (
        <div className="sc-overlay" onClick={() => setLibraryOpen(false)}>
          <div className="sc-modal" onClick={(e) => e.stopPropagation()}>
            <div className="sc-modal-head">
              <div className="sc-heading sc-modal-title">Библиотека навыков</div>
            ...
            <div className="sc-uploadbtn" style={{ marginBottom: 10 }} onClick={() => openUploadFor('library', libTab, null)}>
              <Upload size={15} /> Загрузить в «{QUALITIES.find((q) => q.key === libTab)?.short}»
            </div>
            {library.filter((s) => s.quality === libTab).length === 0 ? (
              <div className="sc-empty-msg">Пока пусто</div>
            ) : (
